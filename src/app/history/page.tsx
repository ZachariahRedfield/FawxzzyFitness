          ...
          {sessions.map((session, index) => (
            <li key={session.id} className="snap-start">
              <Link
                ...
                className="block rounded-lg border border-border bg-[rgb(var(--surface-2)/0.65)] p-3 transition-colors hover:bg-[rgb(var(--surface-2)/0.9)]"
              >
                <p className="text-[11px] font-medium uppercase tracking-wide text-muted">Log #{index + 1}</p>
                ...
                <p className="text-xs text-muted">{new Date(session.performed_at).toLocaleString()}</p>
              </Link>
            </li>
          ))}
          {sessions.length === 0 ? <li className="rounded-lg border border-border bg-[rgb(var(--surface-2)/0.65)] p-3 text-sm text-muted">No sessions yet.</li> : null}
        </ul>
      </div>
      <AppNav />
    </section>
