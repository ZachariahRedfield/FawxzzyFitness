...
- Offline queued set replay now enforces strict FIFO order so later queued items do not sync ahead of an older failed/retrying item.
...
- Preserve deterministic event ordering during reconnect scenarios and avoid out-of-order set ingestion when retries are backoff-gated.
