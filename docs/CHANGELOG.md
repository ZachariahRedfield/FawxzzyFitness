...
### Fixed
WHAT:
- Restored transient removing-state behavior in Session Exercise Focus so removed exercises show immediate “Removing...” feedback and collapse out of the list while server removal is in progress.
WHY:
- This keeps touch interactions responsive and aligns remove-control behavior with expected session list feedback, reducing uncertainty after remove taps.
